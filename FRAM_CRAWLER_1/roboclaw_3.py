...
class Roboclaw:
	...
	def __init__(self, comport, rate, timeout=0.010, retries=1):
...
#		return self._write4444444(address,self.Cmd.SETM2POSPID,long(kd*1024),long(kp*1024),long(ki*1024),kimax,deadzone,min,max)
	...
	def Open(self):
		try:
			self._port = serial.Serial(port=self.comport, baudrate=self.rate, timeout=.02, interCharTimeout=self.timeout)
		except:
			return 0
		return 1
